...
type
  NodeErrorCode* = enum ## Internal error code corresponding libuv.
    ...
    END_RDEND = "read after end"
    END_WRHEAD = "write duplicate head"
    END_WREND = "write after end"
    END_CRLF = "invalid CRLF"
    END_OVERFLOW = "internal buffer overflow"
    END_CHUNKED = "invalid chunked encoding"
    END_EXCESS = "too many data"
    END_UPGRADE = "reuse upgraded request"
    END_FINISHED = "operation not finished"
...
proc newNodeError*(errorCode: NodeErrorCode): ref NodeError =
  ...
  # for code in errorCodes:
  #   if code == errorCode:
  #     result.errorNo = errorNos[i]
  #     break
  #   inc(i)
  # if result.errorNo == 0:
  #   result.errorNo = -10000 # Don't care